...
public class HorizontalPager extends ViewGroup 
{
    ...
    public HorizontalPager(Context context, AttributeSet attrs, int defStyle) {
        ...
        TypedArray a = context.obtainStyledAttributes(attrs, R.styleable.us_eventlocations_androidtab_pager_HorizontalPager);
        pageWidthSpec = a.getDimensionPixelSize(R.styleable.us_eventlocations_androidtab_pager_HorizontalPager_pageWidth, SPEC_UNDEFINED);
        a.recycle();
    ...
    protected void dispatchDraw(Canvas canvas) {
    	...
    	if (mCurrentPage == 1 &&  (getScrollX() < startX - getWidth()/8) )
    	{
        ...
        switch (action) {
            ...
            case MotionEvent.ACTION_MOVE:
            	...
            	{*/
                   ...
                   velocityTracker2.computeCurrentVelocity(1000, mMaximumVelocity);
                   int velocityX2 = (int) velocityTracker2.getXVelocity();

                   //if (velocityX2 > SNAP_VELOCITY && mCurrentPage == 1) {
                   if (x < 700 && mCurrentPage == 1) {
